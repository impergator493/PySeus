...
class TGV(): 
    ...
    def tgv2_denoising(img, alpha0, alpha1, iterations):
        """
        ...
        for it in range(0, iterations):
            ...
            u_list.append(prox_sum_l1(x[0:M*N], img, tau, Wis))
            ...
            p_list.append(np.ravel(proj_ball(p_temp[0:2*M*N].reshape(2, M*N), alpha0)))
            q_list.append(np.ravel(proj_ball(p_temp[2*M*N:6*M*N].reshape(4, M*N), alpha1)))
